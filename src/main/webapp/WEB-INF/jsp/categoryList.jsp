...
<script type="text/javascript">
 	$(document).ready(function(){
 		 $(".tiptop a").click(function(){
 	  	 ...
 	  	 $(".sure").click(function(){
 	        $.ajax({
 				url : "${pageContext.request.contextPath}/deleteCategory?id="+$("#saveId").val(),
 				type : 'post',
 				async : false, 
 				success : function(objs) {
 					if(objs.result==1){
 						window.location.href="${pageContext.request.contextPath}/categoryList";
 					}else{
 						alert(objs.message);
 					}
 				}
 	...
 	function modifyCategory(id) {
 		window.location.href="${pageContext.request.contextPath}/updateCategoryBefore?id="+id;
 	}
 	...
 	function addCategory(){
 		window.location.href="${pageContext.request.contextPath}/addCategoryBefore"
 	}
 	
	function getPage(pageCount){
		window.location.href="${pageContext.request.contextPath}/categoryList?pageCount="+pageCount
	}
	$(function(){
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li onclick="addCategory()"><span ><img src="${pageContext.request.contextPath}/static/images/t01.png" /></span>添加</li>
        </ul>
        ...
        <c:forEach items="${categorys }" var="category">
        	<tbody>
        	 <tr>
		        ...
		        <td>${category.sellMaxamount }</td>
		        <td>${category.sellMinamount }</td>
		        <td><a href="javaScript:void(0)" onclick="modifyCategory('${category.id}')" class="tablelink">修改</a> <a href="javaScript:void(0)" class="tablelink" onclick="delCategory('${category.id}')"> 删除</a></td>
	        </tr> 
	        </tbody>
        </c:forEach>

...
@Controller
public class BuyerAnayController {
	
	@Autowired
	private BuyerService buyerService;
	
	@RequestMapping("/buyeranalysis")
	public ModelAndView stocka(HttpServletRequest request){
		
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("buyeranalysis");
		return modelAndView;
	}
//	List<Category> categorys = categoryService.getAllCategorys();
	
	@RequestMapping("/numAnalyzBuy")
	public @ResponseBody EchartBean stockaAnalyz(HttpServletRequest request){
		EchartBean echartBean = new EchartBean();
		List<Map<String, Object>> citys = buyerService.getBuyerCityCount();
